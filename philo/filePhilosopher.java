...
public class filePhilosopher extends Thread{
    Chopstick left, right;
    int state, identity, myLeft, myRight;
    TableTempFile T;
    File file;

    public filePhilosopher(TableTempFile Tl, int id){
	state = 0;
	identity = id;
	T=Tl;
	myLeft = id-1;
	...
	state = 0;
	try{
		String n = new String("foo")+new Integer(identity).toString();
		file=File.createTempFile(n,".tmp");
//		file.deleteOnExit();
	}catch(IOException e){
		//
	}
